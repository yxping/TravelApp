...

{
	...
	public ClipZoomImageView(final Context context, AttributeSet attrs)
	{
		super(context, attrs);
		this.activity = (ImageActivity)context;
				...
				new SimpleOnGestureListener()
				{
					...
					public boolean onSingleTapConfirmed(MotionEvent e) {
						if(isFullShow) {
							full(false);
							isFullShow = false;
							activity.startBarAnimIn();
						}else {
							full(true);
							isFullShow = true;
							activity.startBarAnimOut();
						}
						return true;
					}
	...
	/**
	 * 设置状态栏的消失和显示，即全屏或有状态栏，可适配api 10
	 * @param enable
	 */
...
//			this.setSystemUiVisibility(View.SYSTEM_UI_FLAG_VISIBLE);
			android.view.WindowManager.LayoutParams attr = activity.getWindow().getAttributes();
			attr.flags &= (~android.view.WindowManager.LayoutParams.FLAG_FULLSCREEN);
